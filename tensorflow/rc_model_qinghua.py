...
class RCModel(object):
    ...
    def _decode_verification(self):
        with tf.variable_scope("Cross_passage_verification"):
            ...
            #tf.batch_mat_mul()
            emb_Q = tf.reshape(self.q_emb , [batch_size , - 1, tf.shape(self.q_emb)[1] , self.vocab.embed_dim ]) #[batch , 5 , seq_len , emb_dim]
            emb_Q = tf.reduce_mean(emb_Q , axis = 2)[0: , 0 , 0:] #[batch , emb_dim]
            emb_Q = tf.expand_dims(emb_Q , 1)#[batch , 1 , emb_dim]
            S_pq = tf.matmul(ver_P , emb_Q , transpose_b=True)#[batch , 5 ,1]
            S_pq = tf.squeeze(S_pq , [-1])
            alpha_for_S = tf.get_variable(name="alpha_for_S",
                                          initializer=tf.constant_initializer(value=1, dtype=tf.float32,
                                                                              verify_shape=True), shape=())
            ...
            S = tf.matrix_set_diag(input = S , diagonal = alpha_for_S * S_pq)
            ...
            g = tf.matmul(RA_concated  , emb_Q , transpose_b=True)
            g = tf.reshape(g, shape= [batch_size , -1]) #[batch , 5 ]
            self.pred_pass_probs = tf.nn.softmax(g , - 1)#[batch , 5 ]
    ...
    def _compute_verification_loss(self):
        ...
        with tf.variable_scope('verification_loss'):
            self.verification_loss = sparse_nll_loss(self.pred_pass_probs, self.real_pass)
    ...
    def evaluate(self, eval_batches, result_dir=None, result_prefix=None, save_full_info=False):
        ...
        for b_itx, batch in enumerate(eval_batches):
            ...
            start_probs, end_probs,yes_probs, pred_pass_probs, loss = self.sess.run([self.start_probs,
                                                          self.end_probs, self.yes_probs, self.pred_pass_probs, self.loss], feed_dict)

            total_loss += loss * len(batch['raw_data'])
            total_num += len(batch['raw_data'])

            padded_p_len = len(batch['passage_token_ids'][0])
            for sample, start_prob, end_prob, yes_prob, pred_pass_prob in zip(batch['raw_data'], start_probs, end_probs , yes_probs , pred_pass_probs):

                best_answer = self.find_best_answer(sample, start_prob, end_prob, yes_prob, pred_pass_prob,padded_p_len)
                if save_full_info:
    ...
    def find_best_answer(self, sample, start_prob, end_prob, yes_prob, verficatioin_scores, padded_p_len):
        """
        Finds the best answer for a sample given start_prob and end_prob for each position.
        This will call find_best_answer_for_passage because there are multiple passages in a sample
        """
        best_p_idx, best_span, best_score = None, None, 0
        for i in range(1,len(yes_prob)):
            yes_prob[i] += yes_prob[i-1]

        for p_idx, passage in enumerate(sample['passages']):
            if p_idx >= self.max_p_num:
                continue
            passage_len = min(self.max_p_len, len(passage['passage_tokens']))
            answer_span, boundary_score = self.find_best_answer_for_passage(
                start_prob[p_idx * padded_p_len: (p_idx + 1) * padded_p_len],
                end_prob[p_idx * padded_p_len: (p_idx + 1) * padded_p_len],
                passage_len)
            #content_score = reduce(lambda a , b : a + b , yes_prob[p_idx * padded_p_len + answer_span[0] : p_idx * padded_p_len + answer_span[1] + 1]) * 1.0 / (answer_span[1] - answer_span[0] + 1)
            content_score = (yes_prob[p_idx * padded_p_len + answer_span[1]] - yes_prob[p_idx * padded_p_len + answer_span[0]]) * 1.0 /(answer_span[1] - answer_span[0] + 1)
            score = boundary_score * content_score * verficatioin_scores[p_idx]
            if score > best_score:
                best_score = score
                best_p_idx = p_idx
